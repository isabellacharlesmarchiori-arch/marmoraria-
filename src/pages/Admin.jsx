    ...
    if (!hasData) return (
        <div className="w-full h-full flex items-center justify-center">
            <div className="text-center">
                <iconify-icon icon="solar:chart-linear" width="28" className="text-zinc-800 block mx-auto mb-2"></iconify-icon>
                <p className="font-mono text-[10px] uppercase text-zinc-700">Sem dados no período</p>
            </div>
        </div>
    );
        ...
        <svg ref={svgRef} viewBox={`0 0 ${VB_W} ${VB_H}`} className="w-full h-full" style={{ overflow: 'visible' }}>
            ...
            {yGridLines.map((gl, i) => (
                <g key={i}>
                    ...
                    <text x={PAD.l - 6} y={gl.y + 3.5} textAnchor="end" fill="#52525b" fontSize="9" fontFamily="monospace">{gl.label}</text>
                </g>
            ))}
            {pts.map((pt, i) => (
                <text key={i} x={pt.x} y={PAD.t + PLOT_H + 18} textAnchor="middle" fill="#52525b" fontSize="9" fontFamily="monospace">{meses[i]}</text>
            ))}
            ...
            {pts.map((pt, i) => (
                <g key={i} onMouseEnter={() => setHoveredIdx(i)} onMouseLeave={() => setHoveredIdx(null)} style={{ cursor: 'default' }}>
                    ...
                    <circle cx={pt.x} cy={pt.y} r={hoveredIdx === i ? 0 : 3} fill="#facc15" stroke="#0a0a0a" strokeWidth="1.5"/>
                </g>
            ))}
            {hoveredIdx !== null && pts[hoveredIdx] && (() => {
                ...
                return (
                    <g>
                        ...
                        <text x={bx + 50} y={pt.y - 22} textAnchor="middle" fill="#a1a1aa" fontSize="9" fontFamily="monospace">{meses[hoveredIdx]}</text>
                ...
                <section className="mb-6 sys-reveal">
                    ...
                    {loading ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-px bg-zinc-800 border border-zinc-800">
                            {[0,1,2,3].map(i => (
                                <div key={i} className="bg-[#0a0a0a] p-5 h-[100px] animate-pulse">
                                    <div className="h-2 w-24 bg-zinc-800 mb-3"></div>
                                    <div className="h-6 w-16 bg-zinc-800"></div>
                                </div>
                            ))}
                        </div>
                    ) : (
                            ...
                            {metricas.map((m, i) => (
                                <div key={i} className="bg-[#0a0a0a] p-5 hover:-translate-y-0.5 transition-all relative group">
                                    ...
                                    <div className="text-3xl font-bold tracking-tighter mb-1 flex items-baseline gap-1">
                                        <span className={m.destaque ? 'text-yellow-400' : 'text-white'}>{m.valor}</span>
                                        {m.unidade && <span className="text-zinc-500 text-lg font-normal">{m.unidade}</span>}
                                    </div>
                                    <div className="font-mono text-[9px] text-zinc-600">{m.detalhe}</div>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <section className="md:col-span-3 sys-reveal sys-delay-100">
                        ...
                        <div className="bg-[#0a0a0a] border border-zinc-800">
                            <div className="grid grid-cols-12 px-4 py-2.5 border-b border-zinc-800">
                                ...
                                <span className="col-span-3 font-mono text-[9px] uppercase tracking-widest text-zinc-600 text-right">Total orçado</span>
                            ...
                            {loading ? (
                                <div className="p-6 space-y-3">
                                    {[0,1,2].map(i => <div key={i} className="h-8 bg-zinc-900 animate-pulse"></div>)}
                                </div>
                            ) : ranking.length === 0 ? (
                                <div className="py-12 text-center">
                                    ...
                                    <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-700">Nenhum dado disponível</p>
                                </div>
                            ) : (
                                <>
                                    {ranking.map((r, i) => (
                                        <div key={r.id} className={`grid grid-cols-12 items-center px-4 py-3 hover:bg-white/[0.015] transition-colors ${i < ranking.length - 1 ? 'border-b border-zinc-900' : ''}`}>
                                            <div className="col-span-4 flex items-center gap-2.5">
                                                <span className={`font-mono text-[9px] w-4 text-right shrink-0 ${i === 0 ? 'text-yellow-400 font-bold' : 'text-zinc-700'}`}>{i + 1}</span>
                                                <div className="w-6 h-6 bg-zinc-800 border border-zinc-700 flex items-center justify-center font-mono text-[8px] text-zinc-400 shrink-0">{r.iniciais}</div>
                                                <span className="text-sm text-white font-medium truncate">{r.nome}</span>
                                            </div>
                                            <div className="col-span-3 text-right">
                                                <span className={`font-mono text-[11px] font-bold ${i === 0 ? 'text-yellow-400' : 'text-white'}`}>{fmtBRL(r.total)}</span>
                                            </div>
                                            <div className="col-span-2 text-right"><span className="font-mono text-[11px] text-zinc-400">{r.orcamentos}</span></div>
                                            <div className="col-span-1 text-right"><span className="font-mono text-[11px] text-zinc-400">{r.fechados}</span></div>
                                            <div className="col-span-2 text-right"><span className="font-mono text-[11px] text-zinc-500">{r.taxa}</span></div>
                                        </div>
                                    ))}
                                            ...
                                            return (
                                                <div key={r.id} className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex-1 h-[3px] bg-zinc-900">
                                                        <div className={`h-full transition-all duration-700 ${i === 0 ? 'bg-yellow-400' : 'bg-zinc-600'}`} style={{ width: `${pct}%` }}></div>
                                                    </div>
                        ...
                        <div className="bg-[#0a0a0a] border border-zinc-800 h-full flex flex-col">
                            <div className="flex items-center justify-between px-4 pt-4 pb-2">
                                ...
                                <div className="text-right">
                                    ...
                                    <div className="font-mono text-sm font-bold text-yellow-400">{fmtBRL(totalPeriodo)}</div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 border-t border-zinc-800">
                                <div className="px-4 py-3 border-r border-zinc-800">
                                    ...
                                    {chartValores[melhorMesIdx] > 0 ? (
                                        <>
                                            <div className="text-white font-semibold text-sm">{chartMeses[melhorMesIdx]}</div>
                                            <div className="font-mono text-[10px] text-yellow-400">{fmtBRL(chartValores[melhorMesIdx])}</div>
                                        </>
                                    ) : <div className="text-zinc-700 font-mono text-[10px]">—</div>}
                                </div>
                                <div className="px-4 py-3">
                                    <div className="font-mono text-[9px] uppercase tracking-widest text-zinc-600 mb-1">Menor mês</div>
                                    {piorMesIdx >= 0 && chartValores[piorMesIdx] > 0 ? (
                                        <>
                                            <div className="text-white font-semibold text-sm">{chartMeses[piorMesIdx]}</div>
                                            <div className="font-mono text-[10px] text-zinc-500">{fmtBRL(chartValores[piorMesIdx])}</div>
                                        </>
                                    ) : <div className="text-zinc-700 font-mono text-[10px]">—</div>}
                                </div>
                            </div>
                ...
                <section className="sys-reveal sys-delay-300">
                    ...
                    <div className="bg-[#0a0a0a] border border-zinc-800">
                        <div className="grid grid-cols-12 px-4 py-2.5 border-b border-zinc-800">
                            ...
                            <span className="col-span-3 font-mono text-[9px] uppercase tracking-widest text-zinc-600">Vendedor</span>
                            <span className="col-span-2 font-mono text-[9px] uppercase tracking-widest text-zinc-600 text-right">Data</span>
                        </div>

                        {loading ? (
                            <div className="p-6 space-y-3">
                                {[0,1,2].map(i => <div key={i} className="h-10 bg-zinc-900 animate-pulse"></div>)}
                            </div>
                        ) : ultimosFechamentos.length === 0 ? (
                            <div className="py-12 text-center">
                                <iconify-icon icon="solar:check-circle-linear" width="28" className="text-zinc-800 block mx-auto mb-2"></iconify-icon>
                                <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-700">Nenhum fechamento registrado</p>
                            </div>
                        ) : (
                            ultimosFechamentos.map((f, i) => (
                                <div key={i} className={`grid grid-cols-12 items-center px-4 py-3.5 hover:bg-white/[0.015] transition-colors ${i < ultimosFechamentos.length - 1 ? 'border-b border-zinc-900' : ''}`}>
                                    <div className="col-span-3 text-sm text-white font-medium truncate pr-2">{f.cliente}</div>
                                    <div className="col-span-4 font-mono text-[10px] text-zinc-500 truncate pr-2">{f.projeto}</div>
                                    ...
                                    <div className="col-span-2 text-right">
                                        <span className="font-mono text-[9px] text-zinc-600">{f.data}</span>
                                    </div>
                                </div>
                            ))
                        )}
