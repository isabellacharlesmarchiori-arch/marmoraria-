...
function PainelDia({ diaKey, medicoes, onClose, onVerDesenho }) {
  ...
  return (
    <div className="mt-4 bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none overflow-hidden">
      <div className="px-4 py-3 border-b border-zinc-200/80 dark:border-zinc-800 flex items-center justify-between">
        <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 capitalize">
          {dataFormatada} · {medicoes.length} medição{medicoes.length !== 1 ? 'ões' : ''}
        </span>
        <button
          onClick={onClose}
          className="w-7 h-7 flex items-center justify-center text-zinc-400 dark:text-zinc-600 hover:text-zinc-900 dark:hover:text-white transition-colors"
        >
          ...
          return (
            <div key={m.id} className="px-4 py-3 space-y-2.5">
              ...
              <div className="min-w-0">
                <div className="text-sm font-semibold text-zinc-900 dark:text-white truncate">{proj.nome ?? '—'}</div>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-widest border ${
                    isProducao
                      ...
                      : 'text-orange-600 dark:text-yellow-400 border-orange-500/30 dark:border-yellow-400/30 bg-orange-500/5 dark:bg-yellow-400/5'
                  }`}>
                    {tipo}
                  </span>
                  <span className="font-mono text-[10px] text-zinc-400 dark:text-zinc-600">{hora}</span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                {svgUrl ? (
                  <button
                    ...
                    className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white border border-zinc-200/80 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-600 px-2.5 py-1.5 transition-colors"
                  >
                  ...
                  <span
                    ...
                    className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-widest text-zinc-300 dark:text-zinc-700 border border-zinc-200/80 dark:border-zinc-900 px-2.5 py-1.5 cursor-not-allowed"
                  >
                    <iconify-icon icon="solar:map-linear" width="11"></iconify-icon>
                    Ver Desenho
                  </span>
  ...
  return (
    <div className="bg-zinc-50 dark:bg-[#050505] text-zinc-500 dark:text-[#a1a1aa] min-h-screen">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-zinc-200/80 dark:border-zinc-800">
        ...
        <h1 className="text-2xl font-semibold text-zinc-900 dark:text-white tracking-tight">Medições Realizadas</h1>
      </div>
        ...
        {mostrarLoading ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-4">
              <button
                onClick={() => navMes(-1)}
                className="w-8 h-8 flex items-center justify-center border border-zinc-200/80 dark:border-zinc-800 text-zinc-500 dark:text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors"
              >
                <iconify-icon icon="solar:alt-arrow-left-linear" width="14"></iconify-icon>
              </button>
              <span className="font-mono text-xs uppercase tracking-widest text-zinc-700 dark:text-zinc-300 capitalize">
                {mesLabel}
              </span>
              <button
                onClick={() => navMes(1)}
            ...
            <div className="bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none overflow-hidden">
              <div className="grid grid-cols-7 border-b border-zinc-200/80 dark:border-zinc-800">
                {DIAS_SEMANA.map(d => (
                  <div key={d} className="py-2 text-center font-mono text-[9px] uppercase tracking-widest text-zinc-400 dark:text-zinc-600">
                    {d}
                  </div>
                ))}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        naoUltLinha ? 'border-b border-zinc-200/80 dark:border-zinc-800' : '',
                        naoUltCol   ? 'border-r border-zinc-200/80 dark:border-zinc-800' : '',
                        temMedicao  ? 'cursor-pointer hover:bg-zinc-200/60 dark:hover:bg-white/[0.02]' : 'cursor-default',
                        isSel       ? 'bg-[#1D9E75]/10' : '',
                      ].join(' ')}
                    >
                      <span className={[
                        'w-6 h-6 flex items-center justify-center font-mono text-[11px] rounded-full',
                        fora   ? 'text-zinc-300 dark:text-zinc-700'
                          : isHoje ? 'bg-[#1D9E75] text-white font-bold'
                          : isSel  ? 'text-zinc-900 dark:text-white'
            ...
            {(() => {
              ...
              return doMes > 0 ? (
                <div className="mt-2 font-mono text-[10px] text-zinc-400 dark:text-zinc-700 text-right">
            ...
            {medicoes.length === 0 && (
              <div className="py-12 text-center">
                <iconify-icon icon="solar:history-linear" width="36" className="text-zinc-200 dark:text-zinc-800 block mx-auto mb-3"></iconify-icon>
                <p className="font-mono text-[11px] uppercase tracking-widest text-zinc-400 dark:text-zinc-700">Nenhuma medição enviada ainda</p>
              </div>
            )}
          </>
        )}
