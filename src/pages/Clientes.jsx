...
function Field({ label, name, type = 'text', defaultValue = '', required = false, span2 = false, textarea = false }) {
  const cls = 'w-full bg-black border border-zinc-800 text-white text-sm px-4 py-3 outline-none focus:border-yellow-400 font-mono resize-none';
  return (
    <div className={span2 ? 'col-span-2' : ''}>
      <label className="text-[9px] uppercase font-mono text-zinc-600 mb-2 block">{label}{required && <span className="text-yellow-400 ml-0.5">*</span>}</label>
      {textarea
...
const statusPedidoColors = {
  FECHADO:   'bg-blue-950 text-blue-300 border-blue-700/60',
  ORCAMENTO: 'bg-zinc-800 text-zinc-400 border-zinc-700',
};
    ...
    <div className="modal-backdrop fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="modal-content bg-[#0a0a0a] border border-zinc-800 w-full max-w-sm p-6 shadow-2xl">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 font-mono text-[10px] uppercase border border-zinc-800 py-3 hover:text-white transition-colors">Cancelar</button>
          <button onClick={onConfirm} className="flex-1 bg-red-500/10 text-red-500 font-mono font-bold text-[10px] uppercase py-3 border border-red-500/30 hover:bg-red-500 hover:text-white transition-all">Excluir</button>
        </div>
      </div>
    </div>
  );
...
function TabClientes({ empresaId, session, isAdmin }) {
  ...
  return (
    <div className="lg:flex lg:gap-8 h-full">
      {/* Lista */}
      <div className={`flex-1 flex flex-col h-full bg-[#020202] border border-zinc-800 ${selected ? 'hidden lg:flex' : 'flex'}`}>
        <div className="p-6 border-b border-zinc-800">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Pesquisar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              className="w-full bg-black border border-zinc-800 text-white text-sm px-10 py-2.5 outline-none focus:border-yellow-400" />
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div key={cli.id} onClick={() => setSelected(cli)}
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className={`font-medium ${selected?.id === cli.id ? 'text-yellow-400' : 'text-white'}`}>{cli.nome}</h3>
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={e => { e.stopPropagation(); openModal(cli); }} className="p-2 border border-zinc-800 text-zinc-500 hover:border-zinc-400 hover:text-white transition-colors">
                    <iconify-icon icon="solar:pen-linear" width="13"></iconify-icon>
                  </button>
                  {isAdmin && (
                    <button onClick={e => { e.stopPropagation(); setDeleteId(cli.id); }} className="p-2 border border-zinc-800 text-zinc-500 hover:border-red-400/50 hover:text-red-400 transition-colors">
      ...
      {/* Ficha */}
      {selected ? (
        <div className="flex-[1.4] bg-[#020202] border border-zinc-800 flex flex-col h-full overflow-hidden sys-reveal relative">
          <button onClick={() => setSelected(null)} className="lg:hidden absolute top-4 left-4 z-20 text-zinc-500 flex items-center gap-2 font-mono text-[10px] uppercase">
          ...
          <div className="p-6 border-b border-zinc-800 bg-black/40">
            <div className="flex justify-between items-start mb-4">
              <div className="text-[10px] font-mono text-zinc-600 border border-zinc-800 px-2 py-1">ID_{selected.id.slice(0, 8)}</div>
              <button onClick={() => openModal(selected)} className="text-[10px] font-mono uppercase bg-zinc-900 px-4 py-2 hover:text-white transition-colors">Editar</button>
            </div>
            <h2 className="text-3xl font-semibold text-white tracking-tighter uppercase mb-5">{selected.nome}</h2>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={label} className="p-3 border border-zinc-800 bg-black/20">
                  <div className="text-[9px] font-mono text-zinc-600 uppercase mb-1">{label}</div>
                  <div className="text-zinc-300 font-mono text-xs">{value || '—'}</div>
                </div>
              ))}
              <div className="col-span-2 p-3 border border-zinc-800 bg-black/20">
                <div className="text-[9px] font-mono text-zinc-600 uppercase mb-1">Endereço</div>
                <div className="text-zinc-300 font-mono text-xs">{selected.endereco || '—'}</div>
              </div>
            </div>
          </div>
          <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
            <h3 className="text-xs font-bold text-white uppercase tracking-widest mb-4 border-l-2 border-yellow-400 pl-3">Projetos</h3>
            {(selected.projetos?.length ?? 0) === 0 ? (
              <div className="py-16 text-center border border-dashed border-zinc-800 text-zinc-700 font-mono text-[10px] uppercase">Nenhum projeto</div>
            ) : selected.projetos.map(proj => (
              <div key={proj.id} className={`p-4 border bg-zinc-950/20 hover:border-zinc-700 transition-colors flex items-center justify-between group/proj mb-2 ${proj.status_pedido === 'FECHADO' ? 'border-blue-900/60' : 'border-zinc-900'}`}>
                <div>
                  <span className="text-zinc-200 font-medium uppercase text-sm">{proj.nome}</span>
                  <span className="text-[10px] font-mono text-zinc-600 uppercase ml-3">Criado {proj.data}</span>
                </div>
                <div className="flex items-center gap-2">
                  {proj.status === 'perdido' && (
                    <button onClick={e => handleReativarProjeto(e, proj)} className="flex items-center gap-1.5 px-2.5 py-1 text-[9px] font-mono uppercase border border-green-500/40 text-green-400 hover:bg-green-400/10 transition-colors">
                      <iconify-icon icon="solar:restart-linear" width="12"></iconify-icon> Reativar
                    </button>
                  )}
                  {/* Badge + botão reverter — só admin vê o botão */}
                    ...
                    <>
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="flex items-center gap-1.5 px-2.5 py-1 text-[9px] font-mono uppercase border border-red-700/40 text-red-400 hover:bg-red-400/10 hover:border-red-500/60 transition-colors"
                        >
        ...
        <div className="hidden lg:flex flex-[1.4] bg-[#020202] border border-zinc-800 items-center justify-center">
          <div className="text-center">
            <iconify-icon icon="solar:users-group-two-rounded-linear" width="48" className="text-zinc-800 mb-4 mx-auto block"></iconify-icon>
            <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-600">Selecione um cliente</p>
          </div>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="bg-[#050505] border border-zinc-800 w-full max-w-lg p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-base font-bold text-white uppercase mb-6 flex items-center gap-3">
            ...
            <form onSubmit={handleSave} className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 flex gap-4 pt-2">
                <button type="button" onClick={closeModal} className="flex-1 border border-zinc-800 text-zinc-400 font-mono text-[10px] uppercase py-3 hover:text-white">Cancelar</button>
                <button type="submit" className="flex-1 bg-white text-black font-mono font-bold text-[10px] uppercase py-3 hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all">Salvar</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {loading ? (
            ...
            <div key={arq.id} onClick={() => setSelected(arq)}
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className={`font-medium ${selected?.id === arq.id ? 'text-yellow-400' : 'text-white'}`}>{arq.nome}</h3>
                  <div className="text-[10px] font-mono text-zinc-600 uppercase mt-1 flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={e => { e.stopPropagation(); openModal(arq); }} className="p-2 border border-zinc-800 text-zinc-500 hover:border-zinc-400 hover:text-white transition-colors">
                    <iconify-icon icon="solar:pen-linear" width="13"></iconify-icon>
                  </button>
                  {isAdmin && (
                    <button onClick={e => { e.stopPropagation(); setDeleteId(arq.id); }} className="p-2 border border-zinc-800 text-zinc-500 hover:border-red-400/50 hover:text-red-400 transition-colors">
                      <iconify-icon icon="solar:trash-bin-trash-linear" width="13"></iconify-icon>
                    </button>
                  )}
                </div>
      ...
      {selected ? (
        <div className="flex-[1.4] bg-[#020202] border border-zinc-800 flex flex-col h-full overflow-hidden sys-reveal relative">
          ...
          <div className="p-6 border-b border-zinc-800 bg-black/40">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {selected.dados_pagamento_pix && (
                <div className="col-span-2 p-3 border border-green-900/40 bg-green-400/5">
                  <div className="text-[9px] font-mono text-green-600 uppercase mb-1">PIX / Dados Bancários</div>
                  <div className="text-green-300 font-mono text-xs">{selected.dados_pagamento_pix}</div>
                </div>
              )}
            </div>
          </div>
          <div className="p-6 flex-1 flex items-center justify-center">
            <div className="text-center">
              <iconify-icon icon="solar:buildings-2-linear" width="36" className="text-zinc-800 mb-3 mx-auto block"></iconify-icon>
              <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-700">Projetos vinculados visíveis na aba Projetos</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="hidden lg:flex flex-[1.4] bg-[#020202] border border-zinc-800 items-center justify-center">
          <div className="text-center">
            <iconify-icon icon="solar:buildings-2-linear" width="48" className="text-zinc-800 mb-4 mx-auto block"></iconify-icon>
            <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-600">Selecione um arquiteto</p>
          </div>
        </div>
      )}
          ...
          <div className="bg-[#050505] border border-zinc-800 w-full max-w-lg p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <label className="text-[9px] uppercase font-mono text-zinc-600 mb-2 block">PIX / Dados Bancários</label>
                <input name="dados_pagamento_pix" defaultValue={editing?.dados_pagamento_pix || ''}
                  placeholder="Chave PIX, agência, conta..."
                  className="w-full bg-black border border-zinc-800 text-white text-sm px-4 py-3 outline-none focus:border-green-500 font-mono placeholder:text-zinc-700" />
              </div>
              <div className="col-span-2 flex gap-4 pt-2">
                <button type="button" onClick={closeModal} className="flex-1 border border-zinc-800 text-zinc-400 font-mono text-[10px] uppercase py-3 hover:text-white">Cancelar</button>
                <button type="submit" className="flex-1 bg-white text-black font-mono font-bold text-[10px] uppercase py-3 hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all">Salvar</button>
...
export default function ClientesPage() {
  ...
  return (
    <div className="page-enter min-h-screen bg-[#050505] text-zinc-400 font-sans selection:bg-white selection:text-black flex flex-col">
      {/* Header */}
      <div className="px-6 pt-6 pb-0 border-b border-zinc-800">
        <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest border border-zinc-800 w-max px-2 py-1 mb-3">06 // Clientes &amp; Arquitetos</div>
