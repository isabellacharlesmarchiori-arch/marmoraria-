...
export default function ContaCard({ conta, onEditar, onDesativar, onRecalcular }) {
  ...
  const corSaldo =
    ...
    saldoNum === 0 ? 'text-zinc-500 dark:text-zinc-500' :
  ...
  return (
    <div className={`bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none p-5 flex flex-col gap-3${conta.ativo ? '' : ' opacity-50'}`}>
      <div className="flex items-center gap-1.5">
        <iconify-icon icon={icone} width="12" className="text-zinc-500 dark:text-zinc-600" />
        <span className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">
          {labelTopo}
        </span>
      </div>

      <p className="text-zinc-900 dark:text-white text-base leading-tight">{conta.nome}</p>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-500 hover:text-orange-600 dark:hover:text-yellow-400 transition-colors"
        >
          Recalcular
        </button>
