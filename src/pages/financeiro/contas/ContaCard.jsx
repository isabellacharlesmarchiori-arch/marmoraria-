...
export default function ContaCard({ conta, onEditar, onDesativar, onRecalcular }) {
  ...
  const corSaldo =
    ...
    saldoNum === 0 ? 'text-gray-500 dark:text-zinc-500' :
  ...
  return (
    <div className={`bg-gray-50 dark:bg-[#0a0a0a] p-5 flex flex-col gap-3${conta.ativo ? '' : ' opacity-50'}`}>
      <div className="flex items-center gap-1.5">
        <iconify-icon icon={icone} width="12" className="text-gray-500 dark:text-zinc-600" />
        <span className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600">
          {labelTopo}
        </span>
      </div>

      <p className="text-gray-900 dark:text-white text-base leading-tight">{conta.nome}</p>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-500 hover:text-yellow-400 transition-colors"
        >
          Recalcular
        </button>
        <span className="text-gray-400 dark:text-zinc-700 select-none">·</span>
        <button
          type="button"
          onClick={onEditar}
          className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-500 hover:text-yellow-400 transition-colors"
