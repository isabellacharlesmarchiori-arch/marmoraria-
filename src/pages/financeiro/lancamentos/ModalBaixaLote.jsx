...
export default function ModalBaixaLote({ aberto, lancamentos, contas, onFechar, onSucesso }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white/95 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 rounded-2xl dark:rounded-none shadow-sm shadow-zinc-100/60 dark:shadow-2xl w-full max-w-lg"
      ...
      >
        ...
        <div className="px-6 pt-5 pb-0 flex flex-col gap-1">
          <div className="flex items-baseline gap-3">
            <span className="text-zinc-900 dark:text-white text-lg font-bold tabular-nums">{elegiveis.length}</span>
        ...
        {elegiveis.length > 0 && (
          <div className="px-6 pt-3 pb-0 max-h-40 overflow-y-auto">
            <div className="border border-zinc-200/80 dark:border-zinc-900 divide-y divide-zinc-200/80 dark:divide-zinc-900">
              {elegiveis.map(l => (
                <div key={l.id} className="flex items-center justify-between px-3 py-2">
                  <span className="text-sm text-zinc-600 dark:text-zinc-300 truncate max-w-[260px]">{l.descricao}</span>
                  <span className="font-mono text-[10px] tabular-nums text-zinc-500 dark:text-zinc-500 shrink-0 ml-3">
