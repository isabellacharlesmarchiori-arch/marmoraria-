...
export default function LinhaCheque({ cheque, onAcao }) {
  ...
  return (
    <tr
      className={`border-b border-zinc-200/80 dark:border-zinc-900 cursor-pointer hover:bg-white/[0.015] transition-colors${vencidoSemDeposito ? ' bg-red-950/20' : ''}`}
      onClick={() => onAcao('detalhe', cheque)}
    >
      {/* Número */}
      <td className="px-4 py-3.5 whitespace-nowrap">
        <span className="font-mono text-sm text-zinc-900 dark:text-white">{cheque.numero_cheque}</span>
      </td>

      {/* Banco + Agência */}
      <td className="px-4 py-3.5">
        <p className="text-sm text-zinc-900 dark:text-white">{cheque.banco_emissor}</p>
        {cheque.agencia_emissora && (
          <p className="font-mono text-[9px] text-zinc-500 dark:text-zinc-600 mt-0.5">
      ...
      <td className="px-4 py-3.5 max-w-[200px]">
        <p className="text-sm text-zinc-900 dark:text-white truncate">{cheque.titular}</p>
        {cheque.documento_titular && (
          <p className="font-mono text-[9px] text-zinc-500 dark:text-zinc-600 mt-0.5">{cheque.documento_titular}</p>
        )}
      </td>
      ...
      {/* Data bom pra */}
      <td className="px-4 py-3.5 whitespace-nowrap">
        <span className={`font-mono text-[11px] ${vencidoSemDeposito ? 'text-red-400' : 'text-zinc-500 dark:text-zinc-500'}`}>
