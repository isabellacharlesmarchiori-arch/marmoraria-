...
function corVariacao(pct) {
  if (pct === null) return 'text-zinc-500 dark:text-zinc-600';
  if (Math.abs(pct) > 10) return pct >= 0 ? 'text-emerald-400' : 'text-red-400';
  return pct >= 0 ? 'text-emerald-600' : 'text-red-600';
}

function CelVar({ pct }) {
  if (pct === null) return <span className="text-zinc-400 dark:text-zinc-700 font-mono text-[9px]">—</span>;
...
export default function AnaliseTrimestral() {
    ...
    return (
      <div className="border border-zinc-200/80 dark:border-zinc-800 bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none p-8 flex flex-col gap-3">
        {Array.from({ length: 9 }).map((_, i) => (
          <div key={i} className="h-4 bg-zinc-100 dark:bg-zinc-900 animate-pulse rounded" style={{ width: `${60 + (i % 4) * 10}%` }} />
        ))}
      </div>
    );
  }
  ...
  const seletorAno = (
    <div className="flex items-center gap-3">
      <label className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">Ano</label>
      <input
        type="number"
        min="2020"
        max="2099"
  ...
  if (!resultados) {
    return (
      <div className="flex flex-col gap-4">
        {seletorAno}
        <div className="border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a] p-12 flex flex-col items-center gap-3">
          <iconify-icon icon="lucide:bar-chart-2" width="28" className="text-zinc-400 dark:text-zinc-700"></iconify-icon>
          <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-center">
            Nenhum dado encontrado para {ano}.
          </p>
          <p className="font-mono text-[8px] text-zinc-400 dark:text-zinc-700 text-center max-w-xs">
          ...
          <button
            type="button"
            onClick={carregar}
            className="font-mono text-[9px] uppercase tracking-widest text-orange-600 dark:text-yellow-400 border border-zinc-200/80 dark:border-zinc-800 px-3 py-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors mt-1"
          >
            Tentar novamente
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Tabela */}
      <div className="border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a] overflow-x-auto">
        ...
        <table className="w-full border-collapse min-w-[640px]">
          <thead>
            <tr className="border-b border-zinc-200/80 dark:border-zinc-800">
              <th className="text-left py-2 pl-4 font-mono text-[8px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">Indicador</th>
              {TRIMS.map(t => (
                <th key={t} className="text-right py-2 pr-3 font-mono text-[8px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">{t}</th>
              ))}
              <th className="text-right py-2 pr-3 font-mono text-[8px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">T1→T2</th>
              <th className="text-right py-2 pr-3 font-mono text-[8px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">T2→T3</th>
              <th className="text-right py-2 pr-3 font-mono text-[8px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">T3→T4</th>
            </tr>
          </thead>
          <tbody>
            {LINHAS.map(linha => {
              ...
              const rowCls = isFinal
                ? 'bg-white dark:bg-zinc-950 border-t-2 border-zinc-200/80 dark:border-zinc-600 border-b border-zinc-200/80 dark:border-zinc-800'
                : isSubtotal
                ? 'bg-zinc-100/60 dark:bg-zinc-950/60 border-t border-zinc-200/80 dark:border-zinc-800 border-b border-zinc-200/80 dark:border-zinc-800'
                : 'border-b border-zinc-900/50 hover:bg-zinc-200/20 dark:hover:bg-zinc-900/20';

              const labelCls = isFinal
                ? 'font-mono text-[10px] font-bold text-zinc-900 dark:text-white uppercase tracking-widest pl-4 py-3'
                : isSubtotal
                ? 'font-mono text-[10px] font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wide pl-4 py-2.5'
                : 'text-sm text-zinc-500 dark:text-zinc-400 pl-4 py-2';

              return (
                <tr key={linha.key} className={rowCls}>
                  <td className={labelCls}>{linha.label}</td>
                  {vals.map((v, i) => {
                    const corV = v < 0 ? 'text-red-400' : isFinal ? 'text-emerald-400' : isSubtotal ? 'text-blue-600 dark:text-blue-300' : 'text-zinc-600 dark:text-zinc-300';
                    return (
                      <td key={i} className={`text-right pr-3 tabular-nums font-mono whitespace-nowrap ${isFinal ? 'py-3 text-base font-bold' : isSubtotal ? 'py-2.5 text-sm font-semibold' : 'py-2 text-sm'} ${corV}`}>
                        {v !== 0 ? formatBRL(Math.abs(v)) : <span className="text-zinc-400 dark:text-zinc-700">—</span>}
                      </td>
                    );
                  })}
            ...
            {(() => {
              ...
              return (
                <tr className="border-b border-zinc-200/80 dark:border-zinc-900/30 bg-zinc-100/30 dark:bg-zinc-950/30">
                  <td className="text-sm text-zinc-500 dark:text-zinc-600 italic pl-4 py-2">% Margem de Contribuição</td>
                  {pctsMC.map((p, i) => (
                    <td key={i} className="text-right pr-3 font-mono text-[10px] tabular-nums text-zinc-500 dark:text-zinc-500 py-2">
                      {p > 0 ? `${p.toFixed(1)}%` : '—'}
                    </td>
                  ))}
                  <td colSpan={3} />
            ...
            {(() => {
              ...
              return (
                <tr className="border-b border-zinc-200/80 dark:border-zinc-900/30 bg-zinc-100/30 dark:bg-zinc-950/30">
                  <td className="text-sm text-zinc-500 dark:text-zinc-600 italic pl-4 py-2">% EBITDA</td>
                  {pctsEBI.map((p, i) => (
                    <td key={i} className="text-right pr-3 font-mono text-[10px] tabular-nums text-zinc-500 dark:text-zinc-500 py-2">
                      {p !== 0 ? `${p.toFixed(1)}%` : '—'}
                    </td>
