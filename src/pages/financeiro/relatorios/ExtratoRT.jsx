...
const STATUS_COR = {
  ...
  cancelado:{ text: 'text-zinc-500 dark:text-zinc-500',    border: 'border-zinc-200/80 dark:border-zinc-800',    label: 'Cancelado' },
};

const INPUT_BASE =
  'bg-white dark:bg-[#0a0a0a] border border-zinc-200/80 dark:border-zinc-800 rounded-md dark:rounded-none px-3 py-2 text-sm text-zinc-900 dark:text-white outline-none ' +
    ...
    <div className="border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a]">
      ...
      >
        <iconify-icon
          ...
          width="14"
          className="text-zinc-500 dark:text-zinc-500 mt-0.5 shrink-0"
        ></iconify-icon>

        <div className="flex-1 min-w-0">
          <p className="text-zinc-900 dark:text-white text-base leading-tight">{arquiteto.nome}</p>
          <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 mt-1">
        ...
        <div className="flex items-center gap-5 shrink-0">
          <div className="text-right">
            <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">Pago</p>
            <p className="font-mono text-sm tabular-nums text-emerald-400">{formatBRL(totalPago)}</p>
          </div>
          <div className="text-right">
            <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">Pendente</p>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-zinc-200/80 dark:border-zinc-800">
                <th className="px-5 py-2.5 font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-left">Data</th>
                <th className="px-5 py-2.5 font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-left">Descrição</th>
                <th className="px-5 py-2.5 font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-right">Valor</th>
                <th className="px-5 py-2.5 font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-left">Status</th>
              </tr>
            </thead>
            <tbody>
              {lancamentos.map(l => {
                ...
                return (
                  <tr key={l.id} className="border-b border-zinc-200/80 dark:border-zinc-900 hover:bg-white/[0.015]">
                    ...
                    <td className="px-5 py-2.5 max-w-xs">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-zinc-900 dark:text-white truncate">{l.descricao}</span>
                        {bloq && (
                          <iconify-icon
                            icon="lucide:lock"
                            width="11"
                    ...
                    <td className="px-5 py-2.5 text-right whitespace-nowrap">
                      <span className="font-mono text-sm tabular-nums text-zinc-900 dark:text-white">
...
export default function ExtratoRT() {
    ...
    return (
      <div className="flex flex-col gap-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="border border-zinc-200/80 dark:border-zinc-800 bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none p-5 flex flex-col gap-2">
            <div className="h-4 w-40 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
            <div className="h-3 w-24 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
            <div className="h-6 w-32 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded mt-1" />
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div className="border border-zinc-200/80 dark:border-zinc-800 p-10 flex flex-col items-center gap-3">
        ...
        <button
          type="button"
          onClick={carregar}
          className="font-mono text-[9px] uppercase tracking-widest text-orange-600 dark:text-yellow-400 hover:text-orange-600 dark:hover:text-yellow-300 border border-zinc-200/80 dark:border-zinc-800 px-3 py-1.5 transition-colors"
        >
          Tentar novamente
        </button>
      </div>
  ...
  if (itens.length === 0) {
    return (
      <div className="border border-zinc-200/80 dark:border-zinc-800 p-10 flex flex-col items-center gap-3">
        <iconify-icon icon="lucide:users" width="28" className="text-zinc-400 dark:text-zinc-700"></iconify-icon>
        <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-400 dark:text-zinc-700 text-center">
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {itensFiltrados.length === 0 ? (
        <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-center py-6">
