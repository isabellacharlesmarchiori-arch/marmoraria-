...
function TooltipCustom({ active, payload, label }) {
  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] border border-zinc-200/80 dark:border-zinc-800 p-3 text-left">
      <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-500 mb-2">{label}</p>
      {payload.map(p => (
...
function BlocoDFC({ bloco, dados, mesAno }) {
  ...
  return (
    <div className={`bg-white dark:bg-[#0a0a0a] border ${bloco.corBg} border p-4 flex flex-col gap-3`}>
      <div className="flex items-start justify-between gap-2">
        <div>
          <iconify-icon icon={bloco.icon} width="14" className={`${bloco.cor} mb-1 block`}></iconify-icon>
          <div className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400">{bloco.titulo}</div>
          <div className="font-mono text-[8px] text-zinc-400 dark:text-zinc-700 mt-0.5">{bloco.desc}</div>
        </div>
      </div>
      <div className="flex flex-col gap-1.5 border-t border-zinc-200/80 dark:border-zinc-800 pt-3">
        <div className="flex justify-between items-center">
          <span className="font-mono text-[9px] text-zinc-500 dark:text-zinc-600">Entradas</span>
          <span className="font-mono text-sm tabular-nums text-emerald-400">+{formatBRL(dados.entradas)}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="font-mono text-[9px] text-zinc-500 dark:text-zinc-600">Saídas</span>
          <span className="font-mono text-sm tabular-nums text-red-400">−{formatBRL(dados.saidas)}</span>
        </div>
        <div className="flex justify-between items-center border-t border-zinc-200/80 dark:border-zinc-800 pt-2 mt-1">
          <span className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-500">Saldo</span>
...
const INPUT_CLS = 'bg-white dark:bg-[#0a0a0a] border border-zinc-200/80 dark:border-zinc-800 px-3 py-1.5 font-mono text-[10px] text-zinc-900 dark:text-white outline-none focus:border-orange-500 dark:focus:border-yellow-400 rounded-md dark:rounded-none transition-colors [color-scheme:dark]';
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
          ...
          <div className="flex items-center gap-1">
            {[['data_vencimento','Vencimento'],['data_pagamento','Caixa']].map(([v, l]) => (
              <button
                ...
                className={`border px-3 py-1 font-mono text-[10px] uppercase tracking-widest cursor-pointer transition-colors ${campoData === v ? 'border-orange-500 dark:border-yellow-400 text-orange-600 dark:text-yellow-400' : 'border-zinc-200/80 dark:border-zinc-800 text-zinc-500 dark:text-zinc-500 hover:border-zinc-300 dark:hover:border-zinc-600'}`}
              >
                {l}
              </button>
            ))}
        ...
        {loadingDFC ? (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-zinc-200 dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-800">
            {[0,1,2].map(i => (
              <div key={i} className="bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none p-4 flex flex-col gap-3">
                <div className="h-3 w-28 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
                <div className="h-5 w-20 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded mt-2" />
                <div className="h-5 w-20 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
              </div>
            ))}
          </div>
        ) : dadosDFC ? (
          <>
            ...
            {(() => {
              ...
              return (
                <div className="flex items-center justify-between px-4 py-3 border border-t-0 border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-950">
        ...
        <p className="font-mono text-[8px] text-zinc-400 dark:text-zinc-700 mt-2">
      ...
      {loading ? (
        <div className="flex flex-col gap-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-zinc-200 dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-800">
            {[0,1,2].map(i => (
              <div key={i} className="bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none p-5 flex flex-col gap-3">
                <div className="h-3 w-20 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
                <div className="h-6 w-28 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
                <div className="h-6 w-28 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
                <div className="h-8 w-32 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded mt-1" />
              </div>
            ))}
          </div>
          <div className="border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a] h-[300px] animate-pulse" />
        </div>
      ) : erro ? (
        <div className="border border-zinc-200/80 dark:border-zinc-800 p-10 flex flex-col items-center gap-3">
          ...
          <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-center">{erro}</p>
          <button type="button" onClick={carregarProjecao} className="font-mono text-[9px] uppercase tracking-widest text-orange-600 dark:text-yellow-400 border border-zinc-200/80 dark:border-zinc-800 px-3 py-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors">
            Tentar novamente
          </button>
        </div>
      ) : dados ? (
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-baseline gap-3">
            <span className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">Saldo atual em contas</span>
            <span className={`text-2xl font-bold tabular-nums tracking-tighter ${dados.saldoAtual >= 0 ? 'text-zinc-900 dark:text-white' : 'text-red-400'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-zinc-200 dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-800">
            {dados.meses.map((m, i) => {
              ...
              return (
                <div key={i} className="bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none p-5 flex flex-col gap-3">
                  <span className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">{m.nome}</span>
                  ...
                  <div className="border-t border-zinc-200/80 dark:border-zinc-800 pt-3">
                    <span className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 block mb-1">Proj. acumulado</span>
          ...
          <div className="border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a]">
            <div className="px-4 py-3 border-b border-zinc-200/80 dark:border-zinc-800">
              <span className="font-mono text-[10px] uppercase tracking-widest text-zinc-900 dark:text-white">Projeção Diária — 30 dias</span>
            </div>
            {dados.pendentes.length === 0 ? (
              <div className="p-10 flex flex-col items-center gap-3">
                <iconify-icon icon="lucide:calendar-x" width="28" className="text-zinc-400 dark:text-zinc-700"></iconify-icon>
                <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600 text-center">Nenhum lançamento pendente nos próximos 90 dias.</p>
              </div>
            ) : (
              <div className="p-4">
