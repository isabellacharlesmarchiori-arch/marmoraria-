...
export default function FinanceiroDashboard() {
  ...
  const inputCls  = 'bg-gray-50 dark:bg-black border border-gray-300 dark:border-zinc-800 text-gray-600 dark:text-zinc-300 text-[11px] font-mono px-3 py-2 rounded-none focus:outline-none focus:border-yellow-400 focus:shadow-[0_0_8px_rgba(250,204,21,0.15)] transition-colors placeholder:text-gray-400 dark:text-zinc-700 w-full';
  ...
  return (
    <div className="p-6 space-y-6 pb-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-800">
      ...
      <div>
        ...
        {erro ? (
          <div className="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-300 dark:border-zinc-800 p-8 text-center">
            ...
            <button onClick={carregarDados} className="font-mono text-[10px] uppercase tracking-widest text-gray-500 dark:text-zinc-400 border border-gray-300 dark:border-zinc-700 px-4 py-2 hover:bg-gray-200 dark:hover:bg-zinc-800 transition-colors">
              Tentar novamente
            </button>
          </div>
        ) : loading ? (
          <div className="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-300 dark:border-zinc-800 divide-y divide-gray-300 dark:divide-zinc-800">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="px-4 py-3.5 flex items-start gap-3">
                <div className="w-2.5 h-2.5 rounded-full bg-gray-200 dark:bg-zinc-800 animate-pulse shrink-0 mt-[5px]" />
                <div className="flex-1 space-y-1.5">
                  <div className="h-3.5 w-52 bg-gray-200 dark:bg-zinc-800 rounded animate-pulse" />
                  <div className="h-3 w-36 bg-gray-200 dark:bg-zinc-800 rounded animate-pulse" />
                </div>
                <div className="space-y-1.5 text-right">
                  <div className="h-3.5 w-24 bg-gray-200 dark:bg-zinc-800 rounded animate-pulse" />
                  <div className="h-3 w-14 bg-gray-200 dark:bg-zinc-800 rounded animate-pulse ml-auto" />
                </div>
              </div>
            ))}
          </div>
        ) : dados.lancamentos.length === 0 ? (
          <div className="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-300 dark:border-zinc-800 p-12 text-center">
            <iconify-icon icon="lucide:inbox" width="32" className="text-gray-400 dark:text-zinc-700 mb-3 block mx-auto"></iconify-icon>
            <p className="font-mono text-[10px] uppercase tracking-widest text-gray-400 dark:text-zinc-700">Nenhuma movimentação nos próximos 15 dias.</p>
          </div>
        ) : (
          <div className="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-300 dark:border-zinc-800 divide-y divide-gray-300 dark:divide-zinc-800">
      ...
      <section className="sys-reveal">
        ...
        <div className="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-300 dark:border-zinc-800 p-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            ...
            <div>
              <label className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 block mb-1.5">Vendedor</label>
              <div className="relative">
                <iconify-icon icon="solar:user-linear" className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-500 dark:text-zinc-600 pointer-events-none" width="13"></iconify-icon>
                <select value={filtroVend} onChange={e => setFiltroVend(e.target.value)} className={selectCls + ' pl-8 pr-7'}>
                  <option value="todos">Todos</option>
                  {vendedoresUnicos.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
                <iconify-icon icon="solar:alt-arrow-down-linear" className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-500 dark:text-zinc-600 pointer-events-none" width="12"></iconify-icon>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-px border border-gray-300 dark:border-zinc-800 w-max">
            {[
              ...
              <button key={tab.id} onClick={() => setFiltroStatus(tab.id)}
                className={`flex items-center gap-2 px-4 py-2 text-[10px] font-mono uppercase tracking-widest transition-colors ${
                  filtroStatus === tab.id ? 'bg-yellow-400 text-black' : 'text-gray-500 dark:text-zinc-600 hover:text-gray-600 dark:hover:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-900'
                }`}>
      ...
      <section className="sys-reveal sys-delay-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-800">
          <div className="bg-gray-50 dark:bg-[#0a0a0a] p-5 relative group hover:-translate-y-0.5 transition-all">
            <iconify-icon icon="solar:wallet-money-linear" width="16" className="text-gray-400 dark:text-zinc-700 absolute top-5 right-5"></iconify-icon>
            <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-2">Total em projetos</div>
            <div className="text-3xl font-bold text-gray-900 dark:text-white tracking-tighter mb-1">
              {loadingProjetos ? <div className="h-9 w-36 bg-gray-200 dark:bg-zinc-800 animate-pulse rounded"></div> : showFechados ? formatBRL(totalRecebido) : '—'}
            </div>
            <div className="font-mono text-[9px] text-gray-500 dark:text-zinc-600">
              {showFechados ? `${nFechamentos} projeto${nFechamentos !== 1 ? 's' : ''} no período` : 'Filtro: apenas perdidos'}
            </div>
          </div>
          <div className="bg-gray-50 dark:bg-[#0a0a0a] p-5 relative group hover:-translate-y-0.5 transition-all">
            <iconify-icon icon="solar:graph-up-linear" width="16" className="text-gray-400 dark:text-zinc-700 absolute top-5 right-5"></iconify-icon>
            <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-2">Ticket médio</div>
            <div className="text-3xl font-bold text-gray-900 dark:text-white tracking-tighter mb-1">
              {loadingProjetos ? <div className="h-9 w-28 bg-gray-200 dark:bg-zinc-800 animate-pulse rounded"></div> : showFechados && nFechamentos > 0 ? formatBRL(ticketMedio) : '—'}
            </div>
            <div className="font-mono text-[9px] text-gray-500 dark:text-zinc-600">por projeto fechado</div>
          </div>
          <div className="bg-gray-50 dark:bg-[#0a0a0a] p-5 relative group hover:-translate-y-0.5 transition-all">
            <iconify-icon icon="solar:check-square-linear" width="16" className="text-gray-400 dark:text-zinc-700 absolute top-5 right-5"></iconify-icon>
            <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-2">Nº de fechamentos</div>
            ...
            <div className="font-mono text-[9px] text-gray-500 dark:text-zinc-600">no período selecionado</div>
          </div>
        </div>
      </section>
        ...
        <section className="sys-reveal sys-delay-200">
          ...
          <div className="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-300 dark:border-zinc-800">
            <div className="grid grid-cols-12 px-4 py-2.5 border-b border-gray-300 dark:border-zinc-800">
              <span className="col-span-1 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600">Data</span>
              <span className="col-span-3 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 pl-1">Cliente</span>
              <span className="col-span-3 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600">Projeto</span>
              <span className="col-span-2 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600">Vendedor</span>
              <span className="col-span-2 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600">Status</span>
              <span className="col-span-1 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 text-right">Valor</span>
            </div>
            {loadingProjetos ? (
              <div className="px-4 py-4 space-y-3">{[1,2,3].map(i => <div key={i} className="h-10 bg-gray-100 dark:bg-zinc-900 animate-pulse rounded"></div>)}</div>
            ) : fechamentosFiltrados.length === 0 ? (
              <div className="px-4 py-12 text-center">
                <iconify-icon icon="solar:wallet-money-linear" width="32" className="text-gray-400 dark:text-zinc-800 mb-3 block mx-auto"></iconify-icon>
                <p className="font-mono text-[10px] uppercase tracking-widest text-gray-400 dark:text-zinc-700">Nenhum projeto fechado no período</p>
              </div>
              ...
              <div key={p.id}
                ...
                } ${i < fechamentosFiltrados.length - 1 ? 'border-b border-gray-200 dark:border-zinc-900' : ''}`}
                onClick={() => setPainelItem(painelItem?.id === p.id ? null : p)}
              >
                <div className="col-span-1 font-mono text-[9px] text-gray-500 dark:text-zinc-600 leading-tight">{formatDate(p.created_at)}</div>
                <div className="col-span-3 pl-1">
                  <span className="text-sm text-gray-900 dark:text-white font-medium truncate block group-hover:text-yellow-400 transition-colors">{p.clienteNome}</span>
                </div>
                <div className="col-span-3">
                  <span className="font-mono text-[10px] text-gray-500 dark:text-zinc-500 truncate block">{p.nome}</span>
                </div>
                <div className="col-span-2 flex items-center gap-1.5">
                  <div className="w-5 h-5 bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 flex items-center justify-center font-mono text-[7px] text-gray-500 dark:text-zinc-400 shrink-0">
                    {p.vendedorNome !== '—' ? p.vendedorNome.split(' ').map(x => x[0]).join('').slice(0, 2) : '??'}
                  </div>
                  <span className="font-mono text-[10px] text-gray-500 dark:text-zinc-500 truncate">{p.vendedorNome.split(' ')[0]}</span>
                </div>
                <div className="col-span-2">
                  <span className={`px-1.5 py-0.5 border font-mono text-[8px] uppercase ${STATUS_COR[p.status] ?? 'text-gray-500 dark:text-zinc-500 border-gray-300 dark:border-zinc-700'}`}>
                    {STATUS_LABEL[p.status] ?? p.status}
                  </span>
                </div>
                <div className="col-span-1 text-right">
                  {p.valor > 0
                    ? <span className="font-mono text-[11px] text-gray-900 dark:text-white font-bold">{formatBRL(p.valor)}</span>
                    : <span className="font-mono text-[11px] text-gray-400 dark:text-zinc-700">—</span>}
                </div>
              </div>
            ))}
            {!loadingProjetos && fechamentosFiltrados.length > 0 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-300 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950">
        ...
        <section className="sys-reveal sys-delay-300">
          ...
          <div className="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-300 dark:border-zinc-800">
            <div className="grid grid-cols-12 px-4 py-2.5 border-b border-gray-300 dark:border-zinc-800">
              ...
              <span className="col-span-2 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600">Vendedor</span>
              <span className="col-span-2 font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 text-right">Valor (orç.)</span>
            </div>
            {loadingProjetos ? (
              <div className="px-4 py-4 space-y-3">{[1,2].map(i => <div key={i} className="h-10 bg-gray-100 dark:bg-zinc-900 animate-pulse rounded"></div>)}</div>
            ) : perdidosFiltrados.length === 0 ? (
              <div className="px-4 py-12 text-center">
                <iconify-icon icon="solar:close-circle-linear" width="32" className="text-gray-400 dark:text-zinc-800 mb-3 block mx-auto"></iconify-icon>
                <p className="font-mono text-[10px] uppercase tracking-widest text-gray-400 dark:text-zinc-700">Nenhum projeto perdido no período</p>
              </div>
            ) : perdidosFiltrados.map((p, i) => (
              <div key={p.id}
                className={`grid grid-cols-12 items-start px-4 py-3.5 hover:bg-white/[0.015] transition-colors ${i < perdidosFiltrados.length - 1 ? 'border-b border-gray-200 dark:border-zinc-900' : ''}`}
              >
                <div className="col-span-1 font-mono text-[9px] text-gray-500 dark:text-zinc-600 pt-0.5">{formatDate(p.created_at)}</div>
                <div className="col-span-3 pl-1">
                  <span className="text-sm text-gray-900 dark:text-white font-medium truncate block">{p.clienteNome}</span>
                </div>
                ...
                <div className="col-span-2 text-right">
                  {p.valor > 0
                    ? <span className="font-mono text-[11px] text-gray-500 dark:text-zinc-400">{formatBRL(p.valor)}</span>
                    : <span className="font-mono text-[11px] text-gray-400 dark:text-zinc-700">—</span>}
                </div>
              </div>
            ))}
      ...
      {painelItem && (
        <>
          ...
          <aside className="fixed right-0 top-0 h-full w-full max-w-[420px] bg-gray-50 dark:bg-[#0a0a0a] border-l border-gray-300 dark:border-zinc-800 z-50 flex flex-col overflow-hidden"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-300 dark:border-zinc-800 shrink-0">
              <div>
                <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-0.5">Detalhes do projeto</div>
                <div className="text-gray-900 dark:text-white font-semibold text-sm truncate max-w-[280px]">{painelItem.clienteNome}</div>
              </div>
              <button onClick={() => setPainelItem(null)} className="text-gray-500 dark:text-zinc-600 hover:text-gray-900 dark:hover:text-gray-900 dark:hover:text-white transition-colors p-1 shrink-0">
                <iconify-icon icon="solar:close-linear" width="18"></iconify-icon>
              </button>
            </div>
            <div className="flex-1 overflow-y-auto">
              <div className="px-5 py-4 border-b border-gray-300 dark:border-zinc-800">
                <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-2">Projeto</div>
                <div className="text-gray-600 dark:text-zinc-300 text-sm">{painelItem.nome}</div>
              </div>
              <div className="grid grid-cols-2 border-b border-gray-300 dark:border-zinc-800">
                <div className="px-5 py-4 border-r border-gray-300 dark:border-zinc-800">
                  <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-2">Valor (orçamentos)</div>
                ...
                <div className="px-5 py-4">
                  <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-2">Data criação</div>
                  <div className="font-mono text-sm font-semibold text-gray-900 dark:text-white">{formatDate(painelItem.created_at)}</div>
                </div>
              </div>
              <div className="px-5 py-4 border-b border-gray-300 dark:border-zinc-800">
                <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-2">Status</div>
                <span className={`px-2 py-1 border font-mono text-[9px] uppercase ${STATUS_COR[painelItem.status] ?? 'text-gray-500 dark:text-zinc-500 border-gray-300 dark:border-zinc-700'}`}>
                  {STATUS_LABEL[painelItem.status] ?? painelItem.status}
                </span>
              </div>
              <div className="px-5 py-4 border-b border-gray-300 dark:border-zinc-800 flex items-center gap-3">
                <div className="w-8 h-8 bg-gray-200 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 flex items-center justify-center font-mono text-[10px] text-gray-500 dark:text-zinc-400 shrink-0">
                  {painelItem.vendedorNome !== '—' ? painelItem.vendedorNome.split(' ').map(p => p[0]).join('').slice(0, 2) : '??'}
                </div>
                <div>
                  <div className="font-mono text-[9px] uppercase tracking-widest text-gray-500 dark:text-zinc-600 mb-0.5">Vendedor</div>
                  <div className="text-gray-900 dark:text-white text-sm font-medium">{painelItem.vendedorNome}</div>
                </div>
              </div>
            </div>
            <div className="px-5 py-4 border-t border-gray-300 dark:border-zinc-800 shrink-0">
              <button onClick={() => setPainelItem(null)}
                className="w-full border border-gray-300 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 font-mono text-[10px] uppercase tracking-widest py-2.5 hover:border-gray-400 dark:hover:border-zinc-500 hover:text-gray-900 dark:hover:text-gray-900 dark:hover:text-white transition-colors flex items-center justify-center gap-2">
