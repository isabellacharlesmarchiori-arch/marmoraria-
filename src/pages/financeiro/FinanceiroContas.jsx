...
export default function FinanceiroContas() {
  ...
  if (profile?.perfil !== 'admin') {
    return (
      <div className="p-6">
        <p className="text-zinc-500 dark:text-zinc-500 font-mono text-sm">
    ...
    <div className="p-6 flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="font-mono text-[9px] uppercase tracking-widest text-orange-600 dark:text-yellow-400 hover:text-orange-600 dark:hover:text-yellow-300 transition-colors border border-zinc-200/80 dark:border-zinc-800 px-3 py-1.5"
        >
          + Nova conta
        </button>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-zinc-200 dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-800">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-white/90 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-sm shadow-zinc-100/60 dark:shadow-none rounded-[2rem] dark:rounded-none p-5 flex flex-col gap-3">
              ...
              <div className="h-9 w-36 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded" />
            </div>
          ))}
        </div>
      ) : contasVisiveis.length === 0 ? (
        <div className="border border-zinc-200/80 dark:border-zinc-800 p-10 flex flex-col items-center gap-3">
          <iconify-icon icon="lucide:landmark" width="28" className="text-zinc-400 dark:text-zinc-700" />
          <p className="font-mono text-[9px] uppercase tracking-widest text-zinc-500 dark:text-zinc-600">
            {mostrarInativas ? 'Nenhuma conta cadastrada' : 'Nenhuma conta ativa'}
          </p>
        </div>
      ) : (
        <div className={`grid gap-px bg-zinc-200 dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-800 ${
