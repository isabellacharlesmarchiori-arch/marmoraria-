...
- Dimensão (mesma lógica do material acima): item cuja ÚNICA fonte nesta página é a própria linha do bloco "LEGENDA/ÍNDICE" — ou seja, sem nenhum bloco de desenho com título/vista própria correspondendo a esse nome ou número NESTA página — NUNCA tem dimensão inventada: fica com dimensoes = "a medir", mesmo que a página traga outros números soltos sem cota nenhuma (ex: numeração de pino de localização num mini-mapa da planta, ou dados do carimbo/rodapé como escala, data, código CAU). Só preencha a dimensão real quando essa peça tiver seu próprio bloco/vista NESTA página, ou quando uma página seguinte trouxer um detalhe que a resolva via atualiza_id
