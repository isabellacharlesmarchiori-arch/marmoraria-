...
- Dimensão (mesma lógica do material acima): item cuja ÚNICA fonte nesta página é a própria linha do bloco "LEGENDA/ÍNDICE", OU um bloco marcado "SEM NENHUM NÚMERO QUE PAREÇA COTA NESTE BLOCO" — NUNCA tem dimensão inventada: fica com dimensoes = "a medir", mesmo que a página traga outros números soltos sem cota nenhuma (ex: numeração de pino de localização num mini-mapa da planta, dados do carimbo/rodapé como escala/data/código CAU, ou uma tabela de esquadrias em outro canto da página). Só preencha a dimensão real quando essa peça tiver seu próprio bloco/vista COM cota NESTA página, ou quando uma página seguinte trouxer um detalhe que a resolva via atualiza_id
- Código de esquadria (P01, P02, J01 etc.) e tabela de esquadrias/portas/janelas NUNCA identificam nem dimensionam uma peça de pedra — esses códigos e a linha "LxH" da tabela descrevem o VÃO/PORTA em si (madeira, alumínio, vidro), não a soleira ou peça de pedra instalada nele. Um bloco "DETALHE" genérico de instalação (ex: "soleira cadeirinha") que só cita esse código + especificação construtiva, sem cota própria, NÃO é a fonte de uma peça nova — segue a regra acima (bloco sem cota → "a medir", nunca um nome/dimensão emprestado da tabela de esquadrias)
...
- Ilha ou bancada com múltiplas faces em pedra (frente, laterais, fundo): extraia UMA peça (saia/lateral) para CADA vista de detalhe DISTINTA e substancial (ex: cada isométrico "DET. ILHA" com seu próprio comprimento e altura) — não fragmente uma única vista em várias peças por causa de cotas menores dentro dela (ver regra acima), e não omita uma vista que tenha cota própria. Mas NÃO crie peça pra uma face (lateral, fundo etc.) que não tenha vista de detalhe própria nem cota explicitamente associada a ELA — mesmo que dê pra CALCULAR uma dimensão plausível combinando números de OUTRAS peças já identificadas (ex: profundidade do tampo + altura da saia frontal formando uma suposta "lateral" de 1,10×0,90): um número que já é a dimensão INTEIRA de outra peça não conta como cota própria dessa peça nova, mesmo que a combinação pareça fazer sentido de marcenaria — existência de peça precisa de fonte própria, igual a qualquer outra. (Isso é só sobre EXISTÊNCIA da peça — não muda a técnica de SOMAR segmentos de cota corrida de uma peça já confirmada, que continua válida normalmente conforme a seção REGRAS PARA SOMAR SEGMENTOS DE COTA abaixo; somar pedaços de UMA peça não é o mesmo que combinar números inteiros de peças DIFERENTES pra fabricar uma peça nova)
...
- Dentro de um bloco, quando os números vierem organizados em "[GRUPO COLINEAR N — eixo ...]", ESSA é a colinearidade real (calculada por código a partir das coordenadas, não é sugestão) — só some números do MESMO grupo colinear. NUNCA some um número de um grupo com um número de outro grupo, nem com um número solto fora de qualquer grupo (mesmo que pareçam próximos no texto)
- Vários grupos colineares (ou números soltos) no MESMO bloco geralmente são vários trechos/arestas da MESMA peça física (ex.: piscina com vários lados retos) — isso é sinal de que a peça é complexa, NÃO uma instrução pra criar um item por grupo. Sem cota totalizadora clara nem indicação EXPLÍCITA de peças fisicamente distintas (ver REGRAS PARA DIMENSIONAMENTO EM PARTES x PEÇAS SEPARADAS acima — nota escrita, materiais/espessuras diferentes, contorno visivelmente descontínuo), retorne UM item só pra essa peça com dimensoes = "a medir" — NUNCA um item por grupo ou por número solto só porque o bloco tem vários candidatos sem cota totalizadora óbvia
- Mesmo dentro do MESMO grupo colinear, dois números só devem ser somados se fizerem sentido como segmentos PARCIAIS de uma medida maior (valores pequenos, tipo recuos/trechos) — dois números já GRANDES e completos no mesmo grupo (ex.: dois valores parecidos com um comprimento inteiro de peça, não um pedaço) são mais prováveis de ser duas medidas distintas (ex.: borda externa x borda interna de uma moldura) do que uma soma — na dúvida, não some: mantenha a peça como UM item só, dimensoes = "a medir" (nunca dois itens separados só por essa dúvida — ver regra acima)
...
- Ambiente NUNCA fica vazio só porque o bloco desta página não repete o nome do cômodo. Antes de deixar ambiente null: (1) procure NESTA página um rótulo de cômodo perto do bloco (título do bloco, ou texto solto de planta baixa); (2) se não achar, veja se esta peça (mesmo nome/tipo) já aparece no CONTEXTO com ambiente preenchido, e reaproveite esse mesmo ambiente — mesmo quando o item for tratado como peça nova por distância de página (ver regra acima), o ambiente correto ainda pode ser reaproveitado do CONTEXTO. Só deixe ambiente null se nenhuma das duas fontes resolver
