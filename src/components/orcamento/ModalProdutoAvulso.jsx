    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 dark:bg-[#0a0a0a] backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 shadow-xl shadow-zinc-200/40 dark:shadow-none rounded-2xl dark:rounded-none w-full max-w-md z-10">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200/80 dark:border-zinc-800">
          <div>
            <div className="text-[9px] font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-600 mb-0.5">[ PRODUTO_AVULSO ]</div>
            <h3 className="text-base font-semibold text-zinc-900 dark:text-white">Adicionar produto</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4">
          {/* Busca */}
          <div>
            <label className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-500 block mb-1.5">Produto</label>
            ...
            <div className="bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 max-h-36 overflow-y-auto">
              {filtrados.map(p => (
                <div
                  key={p.id}
                  onClick={() => handleSelecionar(p)}
                  className={`flex items-center justify-between px-3 py-2 cursor-pointer border-b border-zinc-200/80 dark:border-zinc-900 last:border-b-0 hover:bg-white/[0.02] transition-colors ${prodSel?.id === p.id ? 'bg-orange-50 dark:bg-yellow-400/[0.04]' : ''}`}
                >
                  <div>
                    <div className="text-xs text-zinc-900 dark:text-white">{p.nome}</div>
                    <div className="font-mono text-[9px] text-zinc-500 dark:text-zinc-600">{p.subcategoria}</div>
                  </div>
                  <span className="font-mono text-[10px] text-zinc-500 dark:text-zinc-400">{fmt(p.preco)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {prodSel && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-500 block mb-1.5">Qtd.</label>
                <input
                  type="number"
                  min="1"
                  value={qty}
          ...
          {prodSel && (
            <div className="border border-zinc-200/80 dark:border-zinc-800 bg-zinc-200/50 dark:bg-zinc-950/50 px-3 py-2 flex items-center justify-between">
              <span className="font-mono text-[10px] text-zinc-500 dark:text-zinc-500 uppercase tracking-widest">Subtotal</span>
              <span className="font-mono text-sm text-zinc-900 dark:text-white">{fmt(preco * qty)}</span>
            </div>
          )}

          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onFechar} className="flex-1 border border-zinc-200/80 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 font-mono text-[10px] uppercase tracking-widest py-2.5 hover:border-zinc-400 dark:hover:border-zinc-600 hover:text-zinc-900 dark:hover:text-white transition-colors">
              Cancelar
            </button>
            <button
              type="submit"
